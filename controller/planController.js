const planModel=require('../model/planModel.js');


module.exports.getAllPlans=async function getAllPlans(req,res){
     try{
      let plans=await  planModel.find();
      if(plans){
            return res.json({
                  message:"all plans retrieved",
                  data:plans
            })
      }
      else{
            return res.json({
                  message:'plan not found',
                  
            })
      }
}
catch(err){
      res.status(500).json({
            message:err.message
      });
}
}

module.exports.getPlan=async function getPlan(req,res){
      try{
            let id=req.params.id;
       let plan=await  planModel.findById(id);
       if(plan){
             return res.json({
                   message:" plans retrieved",
                   data:plan
             })
       }
       else{
             return res.json({
                   message:'plan not found',
                   
             })
       }
 }
 catch(err){
       res.status(500).json({
             message:err.message
       });
 }
 }

 module.exports.createPlan=async function createPlan(req,res) {
      try {
        let planobj = req.body;
        console.log("kjfddfkkj"+planobj);
        let createdPlan = await  planModel.create(planobj);
       
         return res.json({
          message: "plan created succesfully",
          data: createdPlan,

        })
      } catch (err) {
        res.status(500).json({
          message: err.message,
        });
      }
    }

    module.exports.newplan=async function newplan(req,res){
      
      try{
          let data=req.body;
            console.log(data);
            let createdPlan = await  planModel.create(data);
            return res.json({
            message:'plan shown succesfully',
            plandata:data
            
      })  
      }
      catch(err){
            res.status(500).json({
                  message:err.message
            });
      }
      
}



 module.exports.deletePlan=async function deletePlan(req,res){
      
      try{
          let id=req.params.id;
            let deletedPlan=await planModel.findByIdAndDelete(id)
            return res.json({
            message:'plan deleted succesfully',
            data:deletedPlan
      })  
      }
      catch(err){
            res.status(500).json({
                  message:err.message
            });
      }
      
}

module.exports.updatePlan = async function updatePlan (req,res) {
      // console.log("req->body"+req.body);
      
      try {
        let id = req.params.id;
        let dataToBeUpdated = req.body;
        console.log(id);
        console.log(dataToBeUpdated);
        let keys = [];
        for (let key in dataToBeUpdated) {
          keys.push(key);
        }
        let plan = await planModel.findById(id);
        for (let i = 0; i < keys.length; i++) {
          plan[keys[i]] = dataToBeUpdated[keys[i]];
        }
        console.log(plan);
        //doc
        await plan.save();
        return res.json({
            message:'plan updated succesfully',
            data:plan
        });
      } catch (err) {
        res.status(500).json({
          message: err.message,
        });
      }
    };

 //get top three plan


module.exports.top3plans=async function top3plans(req,res){
      try{
            const plans=await planModel.find().sort({
                  ratings:-1
            }).limit(3);

            return res.json({
                  message:'top3 plans',
                  data:plans
            })
      }
      catch(err){
            res.status(500).json({
                  message:err.message
            });
      }
    }